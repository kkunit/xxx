  ...
  const handleSend = async (e) => {
    ...
    try {
        ...
        try {
          ...
          setSendError(
            authError?.code === 'auth/operation-not-allowed'
              ? '需要在 Firebase 控制台 → Authentication → Sign-in method 中启用 Anonymous 登录方式，才能向信箱投递留言。'
              : '还在为你连接糖果信箱，请稍后再试一次哦～'
          );
          return;
        }
      ...
      if (error?.code === 'permission-denied') {
        setSendError('Firebase 拒绝了写入请求。请检查 Firestore 规则是否允许匿名用户写入，或者暂时改为测试模式。');
      } else if (error?.code === 'unavailable') {
        setSendError('网络状态不佳或 Firestore 暂时不可用，请稍候再试。');
      } else {
        setSendError('发送遇到了一点小问题，请刷新后再试~');
      }
    } finally {
      setLoading(false);
    }
          ...
          {view === 'write' && (
            <div className="relative">
              ...
              <form onSubmit={handleSend} className="space-y-5">
                ...
                {sendError && (
                  <div className="flex items-start gap-2 text-red-600 bg-red-50 border border-red-200 rounded-2xl px-4 py-3">
                    <Info size={18} className="mt-0.5 text-red-400" />
                    <span className="text-sm leading-relaxed">{sendError}</span>
                  </div>
                )}
